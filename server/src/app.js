...
const word_list = [
  "water","fish","red", "yellow", "green", "blue","car", "house", "tree", "sky", "bottle", "computer", "phone", "plane", "boat", "submarine", "chair", "hat", "plate", "fork", "knife", "hat"
];
...
const makeNewGameState = () => ({
  ...
  red: {
    ...
    cypher: [getRandomWord(),getRandomWord(),getRandomWord(),getRandomWord()],
    players: []
  },
  blue: {
    interceptions: 0,
...
const newTurn = (game_state) => {
  ...
  previousEncryptor = previousEncryptor === undefined? game_state[nextTeam].players[0] : previousEncryptor;

  const teamMemberIds = game_state[nextTeam].players;
  const lastEncryptorIndex = teamMemberIds.findIndex(playerId => playerId === previousEncryptor);
  const nextEncryptorId = teamMemberIds[lastEncryptorIndex + 1 % teamMemberIds.length];
...
io.on('connection', (socket) => {
  ...
  socket.on('disconnect', () => {
   ...
   socket.on('transmission', (transmission) => {
     ...
     if (game_state.history[game_state.history.length -1].transmission !== undefined) {
      console.log("Attempting to transmit again.")
      throw new Error("Can't transmit twice for a single turn")
     }

     game_state.history[game_state.history.length -1].transmission = transmission;

     io.emit('game state', game_state)
   });

   socket.on('guess', ({team, guess}) => {
     console.log(`player ${me.id} has submitted guess ${guess} on behalf of team ${team}`);
     ...
     // Did we get it right?
     const correct = currentTurn.code === guess;
     ...
     if (!correct && !interceptor) {
       ...
       game_state[team].transmission_fails =  game_state[team].transmission_fails + 1;
     }

     // If  both teams have guessed then this is the end of the turn
     if (currentTurn.guesses.red !== undefined && currentTurn.guesses.blue !== undefined) {
